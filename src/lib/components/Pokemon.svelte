<script lang="ts">
  ...
  export let data: AppData;
    ...
    const stringID = `${id < 100 ? id < 10 ? `00${id}` : `0${id}` : `${id}`}${data.pokemonOfTheDay.isShiny ? "S" : ""}`;
  ...
  (urlParams.get('lang') || navigator.language.slice(0, 2)) == 'fr'
    ...
    const PokemonName = data.pokemonOfTheDay.fetched.pokemon.species.names.find(name => name.language.name === userLang).name;
    
    let rename: string = data.pokemonOfTheDay.rename == "" ? PokemonName : data.pokemonOfTheDay.rename;

    const types = data.pokemonOfTheDay.fetched.types.filter(type => type != null)
...
<div class="poke">
  <img src="./images/{stringID}.png" alt="{data.pokemonOfTheDay.fetched.pokemon.pokemon.name}">
  <h2 contenteditable="true" bind:innerHTML={rename} on:keyup={sr}>{data.pokemonOfTheDay.rename}</h2>
  <h4>{PokemonName}</h4>
  <div class="infos">
